...
class GiveawayPlugin(commands.Cog):
        ...
        while True:
            ...
            if g_time <= 0:
                if len(message.reactions) <= 0:
                    ...
                    embed.set_footer(
                        text=f"{giveaway['winners']} {'winners' if giveaway['winners'] > 1 else 'winner'} | Terminado a las"
                    )
                ...
                for r in message.reactions:
                    ...
                    if r.emoji == "🎉":
                        ...
                        await message.edit(embed=embed)
                        await channel.send(
                            f"🎉 ¡Felicidades {winners_text}! Has ganado **{giveaway['item']}**!"
                        )
                        try:
                            self.active_giveaways.pop(str(giveaway["message"]))
                            await self._update_db()
    ...
    async def start(self, ctx: commands.Context, channel: discord.TextChannel):
        ...
        await ctx.send(embed=self.generate_embed("¿Qué se va a sortear?"))
        ...
        await ctx.send(
            embed=self.generate_embed("¿Cuántos ganadores habrá?")
        )
        ...
        if giveaway_winners <= 0:
            await ctx.send(
                "Los sorteos sólo se pueden hacer con uno o más ganadores. Cancelando comando."
            )
            return
        ...
        embed.set_footer(
            text=f"{giveaway_winners} {'winners' if giveaway_winners > 1 else 'winner'} | Termina a las"
        )
    ...
    async def reroll(self, ctx: commands.Context, _id: str, winners_count: int):
        ...
        **Usage:**
        ...
        if _id in self.active_giveaways:
            await ctx.send("Lo siento, pero no puedes hacer reroll a un sorteo activo.")
            return
        ...
        try:
            ...
            await ctx.send("No tengo permiso para leer el historial de mensajes.")
            return
        ...
        for r in message.reactions:
            if r.emoji == "🎉":
                ...
                embed.description = f"¡El sorteo ha terminado!\n\n**{'Ganadores' if winners_count > 1 else 'Ganador'}:** {winners_text}"
                embed.set_footer(
                    text=f"{winners_count} {'winners' if winners_count > 1 else 'winner'} | Ha terminado a las"
                )
                await message.edit(embed=embed)
                await ctx.channel.send(
                    f"🎉 ¡Felicidades {winners_text}! Has ganado **{embed.title}**"
                )
    ...
    async def cancel(self, ctx: commands.Context, _id: str):
        ...
        **Usage:**
        ...
        if _id not in self.active_giveaways:
            await ctx.send("¡No he podido encontrar un sorteo activo con esa ID!")
            return
        ...
        try:
            ...
            await ctx.send("No tengo permisos para leer el historial de mensajes.")
            return
        except discord.NotFound:
            await ctx.send("Mensaje no encontrado.")
            return
        ...
        embed = message.embeds[0]
        embed.description = "El sorteo ha sido cancelado."
        await message.edit(embed=embed)
